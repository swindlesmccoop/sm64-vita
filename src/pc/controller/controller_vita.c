...
static void vita_read(OSContPad *pad) {
    ...
    if (ctrl.buttons & SCE_CTRL_RTRIGGER) pad->button |= R_TRIG;
    ...
    if (ctrl.rx < 60) pad->button |= L_CBUTTONS;
    if (ctrl.rx > 180) pad->button |= R_CBUTTONS;
    if (ctrl.ry < 60) pad->button |= U_CBUTTONS;
    if (ctrl.ry > 180) pad->button |= D_CBUTTONS;
}
